...
def visualize_mask(mask, color="r"):
    im_h, im_w = mask.shape
    mask_img = np.zeros((im_h, im_w, 3), dtype=np.float32)
    mask_img[mask!=0, :3] = to_rgb(color)
    return mask_img

def overlay(img, mask, alpha=0.6):
    indices = (mask!=0)
    img[indices] = img[indices]*alpha + mask[indices] * (1-alpha)
    return img

def collect_masks(masks, color="r", alpha=0.6): # masks should be inference_state["masks"]
    if len(masks) == 0:
        return 
    im_h, im_w = masks[0].squeeze(0).shape
    overlayed = torch.zeros([im_h, im_w])
    color = to_rgb(color)
    for i, mask in enumerate(masks):
        overlayed = overlay(overlayed, mask.squeeze(0).cpu(), alpha)
    return visualize_mask(overlayed, color)
